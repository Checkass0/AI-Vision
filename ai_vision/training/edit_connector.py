...
def handle_left_button(self):
    if not self.dragging:
        if self.keys[pygame.K_LSHIFT]:
            ...
            # start new rect
            self.dragging = True
            rect = pygame.Rect(self.mouse_pos, (0, 0))
            rect.normalize()
        ...
        if self.keys[pygame.K_LSHIFT]:
            ...
            for r in self.rects:
                if r.selected:
                    r.move_ip(*self.get_mouse_delta())
        else:
            # single select mode
            self.rects[-1].move_ip(*self.get_mouse_delta())

    if not any(r.contains(self.mouse_pos) for r in self.rects):
        for r in self.rects:
            r.selected = False
        self.dragging = False

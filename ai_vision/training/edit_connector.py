...
def handle_left_button(self):
    ...
    # ...
    if self.left_mouse_pressed and self.dragging_rectangle is None:
        # Create new rectangle
        new_rect = pygame.Rect(self.start_mouse_pos, (0, 0))
        # ...
        # Loop to select rectangles
        selected_rectangles = []
        while self.left_mouse_pressed:
            mx, my = pygame.mouse.get_pos()
            new_rect.w, new_rect.h = mx - new_rect.x, my - new_rect.y
            # ...
            # Check if shift key is pressed
            if self.keys[pygame.K_LSHIFT]:
                selected_rectangles.append(new_rect)
            pygame.display.flip()
            self.clock.tick(60)
        # End loop
        # Move all selected rectangles when shift is pressed
        if self.keys[pygame.K_LSHIFT]:
            if len(selected_rectangles) > 0:
                dx, dy = self.get_mouse_delta()
                for r in selected_rectangles:
                    r.move_ip(dx, dy)
                self.select_rectangles(selected_rectangles)
        # If shift key is not pressed, select the new rectangle
        else:
            self.select_rectangles([new_rect])

...
class EditConnector:
    ...
    def select_bbox(self, x, y):
        ...
        for i, bbox in enumerate(self.bboxes):
            if x >= bbox.x and x <= bbox.x + bbox.w and y >= bbox.y and y <= bbox.y + bbox.h:
...
class VideoWidget(QWidget):
	...
	def on_mouse(self, event, x, y, flags, param):
		if event == cv2.EVENT_LBUTTONDOWN:
            self.select_bbox(x, y)  # Box auswählen
            if len(self.selected) > 0:
                self.dragging = True
                bbox = self.bboxes[self.selected[0]]
                self.offset_x = x - bbox.x
                self.offset_y = y - bbox.y
        elif event == cv2.EVENT_MOUSEMOVE:
            if self.dragging:
                bbox = self.bboxes[self.selected[0]]
                bbox.x = x - self.offset_x
                bbox.y = y - self.offset_y
        elif event == cv2.EVENT_LBUTTONUP:
            self.dragging = False
	def draw(self):
        for i, bbox in enumerate(self.bboxes):
            if i in self.selected:  # überprüfen, ob die Box ausgewählt ist
                color = (0, 0, 255)  # rote Farbe für ausgewählte Boxen
            else:
                color = (255, 0, 0)  # Standardfarbe für nicht ausgewählte Boxen
            cv2.rectangle(self		
    def add_boxes(self):
